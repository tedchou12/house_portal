...
// dotProject CVS $Id$
// The following lines are modified to correctly load the Library
// from the dotProject distribution
require_once( BETHLEHEM_ROOT_DIR . '/Library/inc/PEAR/Date/TimeZone.php' );
require_once( BETHLEHEM_ROOT_DIR . '/Library/inc/PEAR/Date/Calc.php' );
require_once( BETHLEHEM_ROOT_DIR . '/Library/inc/PEAR/Date/Span.php' );
...
// following line has been modified by Andrew Eddie to support extending the Date class
        //} elseif (is_object($date) && (get_class($date) == 'date')) {
            ...
            $this->copy($date);
        } elseif (preg_match('/\d{4}-\d{2}-\d{2} \d{1,2}:\d{2}:\d{2}/', $date)) {
            $this->setDate($date);
        } elseif (preg_match('/\d{14}/',$date)) {
            $this->setDate($date,DATE_FORMAT_TIMESTAMP);
        } elseif (preg_match('/\d{4}-\d{2}-\d{2} \d{1,2}:\d{2}/', $date)) {
            $this->setDate($date.':00');
        } elseif (preg_match('/\d{4}\/\d{2}\/\d{2} \d{1,2}:\d{2}/', $date)) {
            $this->setDate(str_replace("/", "-", $date).':00');
        } elseif (preg_match('/\d{4}-\d{2}-\d{2}/', $date)) {
            $this->setDate($date.' 00:00:00');
        }  elseif (preg_match('/\d{4}\/\d{2}\/\d{2}/', $date)) {
            $this->setDate(str_replace("/", "-", $date).' 00:00:00');
        }
    ...
    function setDate($date, $format = DATE_FORMAT_ISO)
    {
        switch($format) {
            case DATE_FORMAT_ISO:
                if (preg_match("/([0-9]{4})-([0-9]{2})-([0-9]{2})[ ]([0-9]{1,2}):([0-9]{2}):([0-9]{2})/",$date,$regs)) {
                    $this->year   = $regs[1];
                    $this->month  = $regs[2];
                    $this->day    = $regs[3];
                    $this->hour   = "".sprintf("%02d", $regs[4]);
                    $this->minute = $regs[5];
                    $this->second = $regs[6];
                } else {
                    $this->year   = 0;
